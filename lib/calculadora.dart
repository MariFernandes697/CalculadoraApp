...
class _CalculadoraState extends State<Calculadora> {
  ...
  void _pressionandoBotao(String valor) {
    setState(() {
      if (valor == _limpar) {
        _expressao = '';
        _resultado = '';
  ...
  void _calcularResultado() {
    try {
      _resultado = _avaliandoExpressao(_expressao).toString();
    } catch (e) {
      _resultado = 'Não é possivel calcular';
    }
  }

  double _avaliandoExpressao(String expressao) {
  ...
  Widget _botao(String valor) {
    return TextButton(
      ...
      onPressed: () => _pressionandoBotao(valor),
    );
  }
